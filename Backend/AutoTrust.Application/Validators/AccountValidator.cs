...
namespace AutoTrust.Application.Validators
{
    public class AccountValidator : IAccountValidator
    {
        ...
        {
            if(!await IsEmailUnique(registerDto.Email, cancellationToken))
                return new ValidationResult(false, "Account with this email already exists!");

            if(!await IsPhoneUnique(registerDto.Phone, cancellationToken))
                return new ValidationResult(false, "Account with this phone already exists!");

            return new ValidationResult(true);
        }

        public async Task<ValidationResult> CanChangePhone(int id, string newPhone, CancellationToken cancellationToken)
        {
